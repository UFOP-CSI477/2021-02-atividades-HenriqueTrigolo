...
async function pegaValor1(link){
    const response = await fetch("https://olinda.bcb.gov.br/olinda/servico/PTAX/versao/v1/odata/CotacaoMoedaDia(moeda=@moeda,dataCotacao=@dataCotacao)?@moeda='"+link+"'&@dataCotacao='05-13-2022'&$top=100&$format=json&$select=paridadeCompra")
        .then(response => response.json())
        .then(data => {
...
async function converter(){
    ...
    let valorEntrada = document.getElementById("valor1").value;
    let valorMoeda1 = await Promise.resolve(pegaValor1(m1[0]));
    let valorMoeda2 = await Promise.resolve(pegaValor1(m2[0]));
    
    if(m1[1] == 'A' && m2[1] == 'A'){
        document.getElementById("resultado").value = (valorMoeda2/valorMoeda1)*valorEntrada;
    }
    if(m1[1] == 'A' && m2[1] == 'B'){
        document.getElementById("resultado").value = ((1/valorMoeda2)/valorMoeda1)*valorEntrada;
    }
    if(m1[1] == 'B' && m2[1] == 'A'){
       document.getElementById("resultado").value = (valorMoeda2/(1/valorMoeda1))*valorEntrada;
    }
    if(m1[1] == 'B' && m2[1] == 'B'){
        document.getElementById("resultado").value = (valorMoeda1/valorMoeda2)*valorEntrada;
    }
}